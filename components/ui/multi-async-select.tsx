  ...
  (
    ...
    const handleInputKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === "Enter") {
        ...
        // If the backspace key is pressed and the input is empty, delete the last selected value
    ...
    // Only use optionsRef to record the selected options, and control the size of the optionsRef to reduce the performance impact
    useEffect(() => {
      ...
      if (async) {
        // When initializing, use options to generate optionsRef
        if (!isInit.current) {
          optionsRef.current = temp
          setReserveOptions(temp)
          isInit.current = true
        } else {
          // When options change, only keep the options that exist in the last selectedValues
    ...
    return (
      ...
      >
        <PopoverTrigger asChild>
          <div
            className={cn(
              "border-input data-[placeholder]:text-muted-foreground [&_svg:not([class*='text-'])]:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive flex h-9 w-full min-w-[8rem] items-center justify-between truncate rounded-md border bg-transparent px-3 py-2 text-sm shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50",
              className,
            )}
          >
              ...
              <div className="flex w-full items-center justify-between">
                <div className="flex flex-wrap items-center gap-1 overflow-x-auto">
                  {selectedValues.slice(0, maxCount).map((value) => {
                    ...
                    return (
                      <div
                        className="hover:text-primary dark:hover:text-primary flex h-[20px] items-center gap-1 rounded-sm border border-zinc-200 px-1.5 py-0.5 text-xs text-zinc-600 hover:border-zinc-400 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-zinc-600"
                        key={value}
                      >
                        <div className="flex max-w-[80px] items-center gap-1 truncate">
                          {option?.label}
                        </div>
                        <X
                          className="box-content h-3 w-3 shrink-0 cursor-pointer rounded-full p-0.5 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                          onClick={(event) => {
                  ...
                  {selectedValues.length > maxCount && (
                    <Badge variant="outline" className="h-[20px] px-1.5 py-0.5 text-xs">
                      <span>{`+ ${selectedValues.length - maxCount}`}</span>
                      <X
                        className="hover:text-primary ml-1.5 box-content h-3 w-3 shrink-0 cursor-pointer rounded-full p-0.5 text-zinc-300 hover:bg-zinc-100 dark:text-zinc-500 dark:hover:bg-zinc-800"
                        onClick={(event) => {
                          event.stopPropagation()
                          clearExtraOptions()
                        }}
                ...
                <div className="ml-2 flex items-center justify-between">
                  <X
                    className="hover:text-primary box-content h-4 w-4 shrink-0 cursor-pointer rounded-full p-1 text-zinc-300 hover:bg-zinc-100 dark:text-zinc-500 dark:hover:bg-zinc-800"
                    onClick={(event) => {
                      event.stopPropagation()
                      handleClear()
                    }}
                  ...
                  <ChevronDown className="pointer-events-none ml-2 h-4 w-4 shrink-0 opacity-50" />
                </div>
              </div>
            ) : (
              <div className="flex w-full items-center justify-between">
        ...
        >
          <Command shouldFilter={!async}>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {/* Async mode does not need to select all */}
                {!async && !hideSelectAll && (
                  <CommandItem key="all" onSelect={toggleAll} className="cursor-pointer">
                    <div
                      className={cn(
                        "border-primary mr-2 flex size-4 items-center justify-center rounded-[4px] border shadow-xs transition-shadow outline-none",
