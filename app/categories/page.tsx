...
export const metadata = {
  title: "Categories - Gems Of India",
  description: "Browse gems by category on Gems Of India",
}

// Skeleton for the category header
...
async function CategoryData({
  ...
  const { entities: paginatedEntities, totalCount } = await getEntitiesByCategory(
    categoryId,
    currentPage,
    ITEMS_PER_PAGE,
    sort,
                ...
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <span className="text-muted-foreground bg-secondary rounded-full px-2 py-0.5 text-xs">
                      {countMap.get(category.id) || 0} gems
                    </span>
                  </Link>
                ))}
              </div>
